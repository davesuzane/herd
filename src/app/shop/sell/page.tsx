...
export default function SellPage() {
 ...
 async function handleList(e: React.FormEvent) {
  ...
  if (title.trim().length < 3) { setError('Give it a real title.'); return }
  if (description.trim().length < 10) { setError('Add a bit more description.'); return }
  ...
  if (file) {
    const validationError = validateFile(file, { maxSizeMB: 5, allowedTypes: ['image/png', 'image/jpeg', 'image/webp'] })
    if (validationError) { setError(validationError); setPosting(false); return }

    const filePath = `${user.id}/${crypto.randomUUID()}.${file.name.split('.').pop()?.replace(/[^a-zA-Z0-9]/g, '')}`
  ...
  const { error: insertError } = await supabase.from('shop_items').insert({
    seller_id: user.id, title, description, image_url: imageUrl,
    listing_type: listingType,
    price_cents: listingType === 'stripe' ? Math.round(parseFloat(price) * 100) : 0,
 ...
 // remove the page-level `if (!onboarded) return (...)` block entirely,
// and instead check inside handleList, only for the stripe path:

if (listingType === 'stripe' && !onboarded) {
  setError('Connect your Stripe account first (see the button below) before selling here.')
  return
}
  ...
  <label className="flex items-center gap-2">
    <input type="radio" checked={listingType === 'stripe'} onChange={() => setListingType('stripe')} /> Sell here
  </label>
  <label className="flex items-center gap-2">
    <input type="radio" checked={listingType === 'external'} onChange={() => setListingType('external')} /> Link elsewhere
  </label>
</div>

{listingType === 'stripe' ? (
  <input value={price} onChange={e => setPrice(e.target.value)} placeholder="Price (USD)" type="number" step="0.01"
    className="w-full bg-surface border border-line rounded px-3 py-2.5 text-sm focus:outline-none focus:border-tag transition" />
) : (
  <input value={externalUrl} onChange={e => setExternalUrl(e.target.value)} placeholder="Link to where you're actually selling this"
    className="w-full bg-surface border border-line rounded px-3 py-2.5 text-sm focus:outline-none focus:border-tag transition" />
)}
        <input
          value={price}
