...
type Item = {
  id: string; videoUrl: string; thumbnailUrl: string | null; caption: string | null
  username: string; likeCount: number; liked: boolean; viewCount: number
}

export default function HerdisClient({ items, currentUserId }: { items: Item[]; currentUserId: string | null }) {
  ...
  async function toggleLike(id: string) {
    if (!currentUserId) { router.push('/login?redirect=/herdis'); return }
    const item = list.find(i => i.id === id)
    if (!item) return

    if (item.liked) {
      await supabase.from('herdis_likes').delete().eq('herdi_id', id).eq('user_id', currentUserId)
    } else {
      await supabase.from('herdis_likes').insert({ herdi_id: id, user_id: currentUserId })
    }
    setList(prev => prev.map(i => i.id === id ? { ...i, liked: !i.liked, likeCount: i.likeCount + (i.liked ? -1 : 1) } : i))
  }
  ...
  function pickFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const validationError = validateFile(file, { maxSizeMB: 25, allowedTypes: ['video/mp4', 'video/webm'] })
  ...
  async function confirmUpload() {
    ...
    try {
      ...
      if (duration > 30) { setError('Keep it under 30 seconds.'); return }
    } catch {
    ...
    const ext = pendingFile.name.split('.').pop()?.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() || 'mp4'
    ...
    const { data: videoUrlData } = supabase.storage.from('herdis').getPublicUrl(videoPath)
  ...
  function trackView(id: string) {
    ...
    setList(prev => prev.map(i => i.id === id ? { ...i, viewCount: i.viewCount + 1 } : i))
  }

  return (
    <div className="max-w-md mx-auto">
      <div className="fixed top-[132px] right-4 z-40">
        <button onClick={() => currentUserId ? setShowUpload(true) : router.push('/login?redirect=/herdis')}
      ...
      {showUpload && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-6" onClick={() => setShowUpload(false)}>
          <div className="bg-surface border border-line rounded-xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <h2 className="font-display font-bold text-lg mb-3">Post a Herdi</h2>

            {previewUrl ? (
              <video src={previewUrl} className="w-full rounded-lg mb-3 max-h-64 object-contain bg-black" controls />
            ) : (
              <input type="file" accept="video/mp4,video/webm" onChange={pickFile}
  ...
  return (
    <div className="h-[calc(100vh-132px)] relative flex items-center justify-center bg-black">
      ...
      <div className="absolute bottom-6 left-4 right-16 text-white">
        ...
        <p className="text-[10px] text-white/60 mt-1 font-mono">{item.viewCount} views</p>
      </div>
      ...
      <div className="absolute bottom-6 right-4 flex flex-col items-center gap-4">
        <button onClick={() => onLike(item.id)} className="flex flex-col items-center gap-1">
          <span className={`text-3xl ${item.liked ? '' : 'opacity-60'}`}>{item.liked ? '❤️' : '🤍'}</span>
        ...
        <button onClick={copyShareLink} className="flex flex-col items-center gap-1">
          <span className="text-2xl opacity-80">🔗</span>
          {copied && <span className="text-white text-[9px] font-mono">Copied</span>}
        </button>
      </div>
    </div>
  )
...
function CommentsDrawer({ herdiId, currentUserId, onClose }: { herdiId: string; currentUserId: string | null; onClose: () => void }) {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from('herdis_comments').select('id, user_id, body, created_at').eq('herdi_id', herdiId).order('created_at')
      setComments(data || [])
  ...
  async function postComment(e: React.FormEvent) {
    ...
    const { data } = await supabase.from('herdis_comments').insert({ herdi_id: herdiId, user_id: currentUserId, body }).select().single()
    if (data) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end" onClick={onClose}>
      <div className="bg-surface w-full max-h-[70vh] rounded-t-xl p-4 flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 mb-3">
          {comments.map(c => (
            <div key={c.id} className="text-sm">
              <span className="font-mono text-xs text-tag mr-2">{usernames[c.user_id] ?? '…'}</span>
              {c.body}
            </div>
          ))}
        ...
        <form onSubmit={postComment} className="flex gap-2">
          ...
          <button type="submit" className="text-xs font-mono bg-tag text-[#1a2015] font-semibold px-4 rounded hover:brightness-110 transition">Post</button>
        </form>
      </div>
    </div>
  )
...
function SendToDmModal({ herdiId, currentUserId, onClose }: { herdiId: string; currentUserId: string | null; onClose: () => void }) {
  const supabase = createClient()
  const router = useRouter()
  ...
  async function send(e: React.FormEvent) {
    ...
    const { data: target } = await supabase.from('profiles').select('id, username').eq('username', username.trim()).maybeSingle()
    if (!target) { setStatus('No user with that username.'); return }

    const [userA, userB] = [currentUserId, target.id].sort()
    const { data: existing } = await supabase.from('dm_conversations').select('id').eq('user_a', userA).eq('user_b', userB).maybeSingle()

    let conversationId = existing?.id
    if (!conversationId) {
      const { data: created, error } = await supabase.from('dm_conversations').insert({ user_a: userA, user_b: userB }).select().single()
    ...
    await supabase.from('chat_messages').insert({
      conversation_id: conversationId, sender_id: currentUserId, body: `Sent a Herdi: ${link}`,
    })

    setStatus('Sent ✓')
    setTimeout(() => { onClose(); router.push(`/chat?dm=${target.username}`) }, 700)
  }

  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-6" onClick={onClose}>
      <form onSubmit={send} className="bg-surface border border-line rounded-xl p-6 max-w-xs w-full space-y-3" onClick={e => e.stopPropagation()}>
        ...
        <button type="submit" className="w-full text-xs font-mono bg-tag text-[#1a2015] font-semibold py-2 rounded hover:brightness-110 transition">
          Send
        </button>
      </form>
    </div>
